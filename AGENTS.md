...
- `Src/Server/Core/Simulation.luau` is the server composition and lifecycle owner. It uses and starts the module-global SoundService, DebugService, AgentService, and EnvironmentService singletons, selects Agent teams and spawn placement, starts Agents, and shuts services down before caller-owned Agent/model teardown.
- `Src/Server/Agents/Agent.luau` owns one Agent's runtime state, controller reference, VisionSensor and SoundSensor outputs, inventory, planner module, cadence, and Heartbeat. It passes AgentService to VisionSensor; VisionSensor requires EnvironmentService directly, while SoundSensor requires SoundService directly. Module-global service singletons are allowed where they do not create require cycles.
...
- `Src/Server/Services/SoundService.luau` is a module-global singleton that records short-lived `AgentFired` BindableEvent signals and exposes opposing-team shot positions. Simulation owns its Start/End lifecycle.
...
| `Src/Server/Agents/Sensors/SoundSensor.luau` | [SoundSensor.md](C:/Users/Leo/Documents/files/Obsiddy/CompyAgent/Docs/SoundSensor.md) | Keep construction, hearing filters, shared-data output, and Agent cadence integration synchronized. |
...
| `Src/Server/Services/SoundService.luau` | [SoundService.md](C:/Users/Leo/Documents/files/Obsiddy/CompyAgent/Docs/SoundService.md) | Keep the `AgentFired` signal contract, public methods, lifetime filtering, singleton ownership, and Simulation lifecycle synchronized. |
